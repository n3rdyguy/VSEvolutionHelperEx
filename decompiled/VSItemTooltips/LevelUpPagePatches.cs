...
public static class LevelUpPagePatches
{
	public static void Show_Postfix(LevelUpPage __instance)
	{
		...
		{
			MelonLogger.Error($"Error in LevelUpPage.Show patch: {arg}");
		}
	}
	...
	private static void TryCacheGameSessionFromLevelUpPage(LevelUpPage page)
	{
		...
		foreach (string text in array3)
		{
			...
			if (property != null)
			{
				...
				{
					MelonLogger.Warning("Error accessing " + text + " property: " + ex.Message);
				}
			}
			...
			{
				MelonLogger.Warning("Error accessing " + text + " field: " + ex2.Message);
			}
		}
